...
export class RecommendationEngine {
  ...
  async forOpportunity(opp: {
    ...
    // Opportunity in PROSPECTING > 7 days — move to PROPERTY_TOUR or schedule meeting
    if (stageLower === "prospecting" && lastActivity > 7) {
      recs.push({
  ...
  private async recommendMoveStuckDeals(orgId: string, ownerId?: string): Promise<Recommendation[]> {
    ...
    const stuck = await prisma.opportunity.findMany({
      where: {
        organizationId: orgId,
        ...(ownerId ? { ownerId } : {}),
        stage: { in: ["PROSPECTING", "prospecting", "INITIAL_CONTACT", "initial_contact", "PROPERTY_TOUR", "property_tour"] },
