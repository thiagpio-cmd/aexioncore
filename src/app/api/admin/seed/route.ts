...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const aexionAdmin = await mkUser("admin@aexionrealty.com",     "Victoria Sterling",  "ADMIN",   "EXECUTIVE", execTeam.id, adminPw);
    const sarah       = await mkUser("sarah@aexionrealty.com",     "Sarah Chen",         "ADMIN",   "EXECUTIVE", execTeam.id, demoPw);
    const mike        = await mkUser("mike@aexionrealty.com",      "Mike Harrison",      "MANAGER", "MANAGER",   mgrTeam.id,  demoPw);
    ...
    await prisma.meeting.createMany({
      data: [
        ...
        { organizationId: O, title: "Q1 pipeline review — sales leadership",                    ownerId: mike.id,   contactId: ctCatherinePalm.id,                                      startTime: d(3, 10),     endTime: d(3, 12),      location: "Aexion Realty HQ — Conference Room B",       notes: "Pipeline strong at $47M. Focus on converting Offer-stage deals. Need comp data for 2 properties.",  attendees: JSON.stringify(["Mike Harrison", "Sarah Chen", "Emma Blackwell", "James Whitfield"]) },
    ...
    return Response.json({
      ...
      credentials: {
        admin: { email: "admin@aexionrealty.com", password: "aexion123" },
        demo: [
          { email: "sarah@aexionrealty.com", password: "demo123", role: "ADMIN" },
          { email: "mike@aexionrealty.com", password: "demo123", role: "MANAGER" },
