...
/**
 * GET /api/integrations/callback/google-calendar
 *
 * Static OAuth callback for Google Calendar. Google requires a fixed redirect URI,
...
export async function GET(request: NextRequest) {
  ...
  console.log("[Google Calendar Callback] Received callback. code:", !!code, "state:", !!state, "error:", oauthError);
  ...
  try {
    ...
    if (state) {
      try {
        ...
        if (sigBuffer.length !== expectedBuffer.length || !timingSafeEqual(sigBuffer, expectedBuffer)) {
          console.error("[Google Calendar Callback] HMAC signature verification failed");
        ...
        console.log("[Google Calendar Callback] Verified state:", JSON.stringify(statePayload));
    ...
    if (!code || !statePayload) {
      console.error("[Google Calendar Callback] Missing code or state. code:", !!code, "statePayload:", !!statePayload);
      return NextResponse.redirect(
    ...
    console.log("[Google Calendar Callback] Looking up integration:", integrationId);

    // Look up the integration
    const integration = await prisma.integration.findUnique({
      where: { id: integrationId },
    });
    console.log("[Google Calendar Callback] Integration found:", !!integration, integration?.id, integration?.status);
    if (!integration) {
      return NextResponse.redirect(
    ...
    // Get the provider
    const providerKey = integration.providerKey ?? "google-calendar";
    const provider = providerRegistry.get(providerKey);
    console.log("[Google Calendar Callback] Provider found:", !!provider, "key:", providerKey);
    if (!provider) {
      return NextResponse.redirect(
    ...
    }).catch((err) => console.error("[Google Calendar Auto-Sync] Failed to trigger:", err));

    return NextResponse.redirect(
      new URL(`${redirectPath}?connected=true`, request.url)
    );
  } catch (error: unknown) {
    console.error("GET /api/integrations/callback/google-calendar error:", error);
