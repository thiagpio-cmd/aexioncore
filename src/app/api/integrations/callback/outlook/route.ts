...
/**
 * GET /api/integrations/callback/outlook
 *
 * Static OAuth callback for Outlook / Microsoft 365.
 * Microsoft requires a fixed redirect URI, so this route serves as the
 * stable endpoint. The integration ID is extracted from the `state`
 * query param (base64url-encoded JSON, HMAC-signed).
 */
export async function GET(request: NextRequest) {
  ...
  console.log("[Outlook Callback] Received callback. code:", !!code, "state:", !!state, "error:", oauthError);
  ...
  try {
    ...
    if (state) {
      try {
        ...
        if (sigBuffer.length !== expectedBuffer.length || !timingSafeEqual(sigBuffer, expectedBuffer)) {
          console.error("[Outlook Callback] HMAC signature verification failed");
        ...
        console.log("[Outlook Callback] Verified state:", JSON.stringify(statePayload));
    ...
    if (!code || !statePayload) {
      console.error("[Outlook Callback] Missing code or state. code:", !!code, "statePayload:", !!statePayload);
      return NextResponse.redirect(
    ...
    console.log("[Outlook Callback] Looking up integration:", integrationId);

    // Look up the integration
    const integration = await prisma.integration.findUnique({
      where: { id: integrationId },
    });
    console.log("[Outlook Callback] Integration found:", !!integration, integration?.id, integration?.status);
    if (!integration) {
      return NextResponse.redirect(
    ...
    // Get the provider
    const providerKey = integration.providerKey ?? "outlook";
    const provider = providerRegistry.get(providerKey);
    console.log("[Outlook Callback] Provider found:", !!provider, "key:", providerKey);
    if (!provider) {
      return NextResponse.redirect(
    ...
    }).catch((err) => console.error("[Outlook Auto-Sync] Failed to trigger:", err));

    return NextResponse.redirect(
      new URL(`${redirectPath}?connected=true`, request.url)
    );
  } catch (error: unknown) {
    console.error("GET /api/integrations/callback/outlook error:", error);
