 ...
 *
 * Static OAuth callback for Slack. Slack requires a fixed redirect URI,
 * so this route serves as the stable endpoint. The integration ID is
 * extracted from the `state` query param (base64url-encoded JSON with HMAC).
 */
export async function GET(request: NextRequest) {
  ...
  console.log("[Slack Callback] Received callback. code:", !!code, "state:", !!state, "error:", oauthError);
  ...
  try {
    ...
    if (state) {
      try {
        ...
        if (sigBuffer.length !== expectedBuffer.length || !timingSafeEqual(sigBuffer, expectedBuffer)) {
          console.error("[Slack Callback] HMAC signature verification failed");
        ...
        console.log("[Slack Callback] Verified state:", JSON.stringify(statePayload));
    ...
    if (!code || !statePayload) {
      console.error("[Slack Callback] Missing code or state. code:", !!code, "statePayload:", !!statePayload);
      return NextResponse.redirect(
    ...
    console.log("[Slack Callback] Looking up integration:", integrationId);

    // Look up the integration
    const integration = await prisma.integration.findUnique({
      where: { id: integrationId },
    });
    console.log("[Slack Callback] Integration found:", !!integration, integration?.id, integration?.status);
    if (!integration) {
      return NextResponse.redirect(
    ...
    // Get the provider
    const providerKey = integration.providerKey ?? "slack";
    const provider = providerRegistry.get(providerKey);
    console.log("[Slack Callback] Provider found:", !!provider, "key:", providerKey);
    if (!provider) {
      return NextResponse.redirect(
    ...
    console.error("GET /api/integrations/callback/slack error:", error);
