...
const FIT_STYLES: Record<FitTier, { label: string; color: string; bg: string }> = {
  ...
  HIGH_FIT: { label: "Alto Fit", color: "text-[var(--accent,#2457FF)]", bg: "bg-[var(--accent,#2457FF)]/10" },
  MEDIUM_FIT: { label: "Medio", color: "text-amber-400", bg: "bg-amber-400/10" },
  LOW_FIT: { label: "Baixo", color: "text-[var(--text-muted,#888)]", bg: "bg-[var(--text-muted,#888)]/10" },
...
function ScoreBar({ label, value }: { label: string; value: number }) {
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-[10px] text-[var(--text-muted,#888)] w-8 shrink-0">{label}</span>
      <div className="flex-1 h-1 rounded-full bg-[var(--border-subtle,rgba(255,255,255,0.06))] overflow-hidden">
        <div
          className="h-full rounded-full bg-[var(--accent,#2457FF)]"
          style={{ width: `${Math.min(100, value)}%` }}
        />
      </div>
      <span className="text-[10px] font-medium text-[var(--text-primary,#fff)] w-5 text-right">
        {Math.round(value)}
      </span>
    </div>
  );
...
function SkeletonRow() {
  return (
    <div className="animate-pulse flex items-center gap-4 p-3">
      <div className="h-8 w-8 rounded-full bg-[var(--border-subtle,rgba(255,255,255,0.06))]" />
      <div className="flex-1 space-y-1">
        <div className="h-3 w-32 rounded bg-[var(--border-subtle,rgba(255,255,255,0.06))]" />
        <div className="h-2.5 w-24 rounded bg-[var(--border-subtle,rgba(255,255,255,0.06))]" />
      </div>
      <div className="h-6 w-16 rounded bg-[var(--border-subtle,rgba(255,255,255,0.06))]" />
    </div>
  );
}
...
export function ICPMatchList({ icpId }: ICPMatchListProps) {
  ...
  if (!data || data.matches.length === 0) {
    return (
      <GlassCard className="p-6 text-center">
        <svg className="mx-auto h-10 w-10 text-[var(--text-muted,#888)] mb-2" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
    ...
    return (
      <div className="space-y-3">
        <button
          ...
          className="flex items-center gap-1.5 text-xs text-[var(--accent,#2457FF)] hover:underline"
        >
  ...
  return (
    <div className="space-y-2">
      ...
      <GlassCard className="p-0 overflow-hidden divide-y divide-[var(--border-subtle,rgba(255,255,255,0.06))]">
        {data.matches.map((m) => {
          ...
          return (
            <button
              ...
              className="w-full flex items-start gap-3 p-3 text-left transition-colors hover:bg-[rgba(255,255,255,0.03)]"
            >
              {/* Score circle */}
              <div
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-sm font-medium text-[var(--text-primary,#fff)] truncate">
                    {m.contactName}
                  </span>
                  <span
                ...
                {m.companyName && (
                  <p className="text-xs text-[var(--text-muted,#888)] truncate">
                    {m.companyName}
                  </p>
                )}
                ...
                {m.match.signals.length > 0 && (
                  <div className="mt-1 flex flex-wrap gap-1">
                    {m.match.signals.map((s, i) => (
                      <span
                        key={i}
                        className="text-[10px] text-[var(--text-muted,#888)] bg-[var(--glass-bg,rgba(255,255,255,0.03))] rounded px-1.5 py-0.5"
                      >
                        {s}
                      </span>
                    ))}
              ...
              {/* Chevron */}
              <svg
                className="shrink-0 mt-2 text-[var(--text-muted,#888)]"
                width="14"
                height="14"
                viewBox="0 0 24 24"
                fill="none"
